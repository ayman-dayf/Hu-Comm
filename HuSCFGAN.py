...
def train_step(
    ...
    clients_Disc2,
    server_disc,
    clients_Gen1_optimizer,
    clients_Gen2_optimizer,
    server_gen_optimizer,
    clients_Disc2_optimizer,
    clients_Disc1_optimizer,
    server_disc_optimizer,
    real_images,
    device,
    client_cuts,
    z_dim=100,
    ...
    real_labels = [
        torch.ones(batch_sizes[i], 1).to(device) for i in range(len(batch_sizes))
    ]
    fake_labels = [
        torch.zeros(batch_sizes[i], 1).to(device) for i in range(len(batch_sizes))
    ]
    # print(real_images)
    # --- Train Discriminator ---
    ...
    d_losses_real, cluster_labels, kld_scores_dict, global_kld_scores_dict = (
        discriminate(
            batch_sizes,
            labels,
            clients_Disc1,
    ...
    for opt in clients_Gen2_optimizer:
        opt.step()
    return (
        torch.stack(d_losses_real).mean(),
        torch.stack(d_losses_fake).mean(),
...
def discriminate(
    ...
    if not cluster:
        server_disc_outputs = server_disc(
            client_disc1_outputs_0,
            client_disc1_outputs_1,
            d1_participants,
            d2_participants,
        ...
        server_disc_outputs, cluster_indeces, cluster_input = server_disc(
            client_disc1_outputs_0,
            client_disc1_outputs_1,
            d1_participants,
            d2_participants,
        ...
        cluster_labels, kld_scores_dict, global_kld_scores_dict = cluster_tensors(
            cluster_input, n_clusters, cluster_indeces
        )
    clients_disc2_outputs = [None] * len(clients_Disc2)
    for i in range(len(clients_Disc2)):
        clients_disc2_outputs[i] = criterion(
            clients_Disc2[i](server_disc_outputs[i]), labels[i]
        )
    return (
        clients_disc2_outputs,
        cluster_labels,
        kld_scores_dict,
        global_kld_scores_dict,
    )
...
def compute_kld_scores(tensor_list):
    ...
    for i in range(user_probs.shape[0]):
        ...
        global_distribution = (user_probs.sum(dim=0) - P) / (
            user_probs.shape[0] - 1
        )  # Mean of others
        ...
        kld = F.kl_div(
            P_safe.log(), global_safe, reduction="batchmean"
...
def federated_averaging(
    ...
    if len(global_scores) > 0:
        for i in range(len(client_sizes)):
            global_total_score = global_total_score + client_sizes[i] * np.exp(
                -1 * alpha * global_scores[i]
            )
    ...
    for i in range(len(all_layers_disc2)):
        ...
        for j in range(len(client_sizes)):
            ...
            if len(global_scores) == 0:
                ...
                score = (
                    client_sizes[j]
                    * np.exp(-1 * alpha * global_scores[j])
                    / global_total_score
                )
            ...
            if len(client_Disc1[j].state_dict()) > i:
                layer_sum += (
                    score
                    * client_Disc1[j].state_dict()[
                        list(client_Disc1[j].state_dict().keys())[i]
                    ]
                )
            elif len(all_layers_disc2) - len(client_Disc2[j].state_dict()) > i:
                layer_sum += (
                    score * server_Disc.state_dict()[list(all_layers_disc2.keys())[i]]
                )
            else:
                layer_sum += (
                    score
                    * client_Disc2[j].state_dict()[
                        list(client_Disc2[j].state_dict().keys())[
                            i
                            - (
                                len(all_layers_disc2)
                                - len(client_Disc2[j].state_dict())
                            )
                        ]
                    ]
                )
    ...
    for i in range(len(all_layers_gen2)):
        ...
        for j in range(len(client_sizes)):
            ...
            if len(client_GENs1[j].state_dict()) > i:
                layer_sum += (
                    score
                    * client_GENs1[j].state_dict()[
                        list(client_GENs1[j].state_dict().keys())[i]
                    ]
                )
            elif len(all_layers_gen2) - len(client_GENs2[j].state_dict()) > i:
                layer_sum += (
                    score * server_GEN.state_dict()[list(all_layers_gen2.keys())[i]]
                )
            else:
                # print("me: "+list(client_GENs2[j].state_dict().keys())[i-(len(all_layers_gen2) - len(client_GENs2[j].state_dict()))])
                layer_sum += (
                    score
                    * client_GENs2[j].state_dict()[
                        list(client_GENs2[j].state_dict().keys())[
                            i
                            - (len(all_layers_gen2) - len(client_GENs2[j].state_dict()))
                        ]
                    ]
                )
    ...
    for label in set(labels.values()):
        ...
        for idx, val in labels.items():
            if val == label:
                ...
                if len(clients_scores) > 0:
                    total_score[label] = total_score[label] + client_sizes[
                        idx
                    ] * np.exp(
                        -1 * alpha * clients_scores[idx]
                    )  # Accumulate sum
        # print(sizes)
            ...
            for j in range(len(client_sizes)):
                if labels[j] == label:
                    if len(clients_scores) == 0 or sizes == client_sizes[j]:
                        ...
                        score = (
                            client_sizes[j] * np.exp(-1 * alpha * clients_scores[j])
                        ) / total_score[label]
                    ...
                    if len(client_GENs1[j].state_dict()) > i:
                        ...
                        layer_sum += (
                            score
                            * server_GEN.state_dict()[list(all_layers_gen.keys())[i]]
                        )
                    else:
            ...
            for j in range(len(client_sizes)):
                if labels[j] == label:
                    ...
                    if len(client_Disc1[j].state_dict()) > i:
                        ...
                        layer_sum += (
                            score
                            * server_Disc.state_dict()[list(all_layers_disc.keys())[i]]
                        )
                    else:
                        layer_sum += (
                            score
        ...
        for i in range(len(client_sizes)):
            if labels[i] == label:
                ...
                zip1 = zip(
                    client_GENs1[i].state_dict().keys(),
                    list(all_layers_gen.values())[: len(client_GENs1[i].state_dict())],
                )
                zip1 = dict(zip1)
                client_GENs1[i].load_state_dict(zip1)
                zip1 = zip(
                    client_GENs2[i].state_dict().keys(),
                    list(all_layers_gen.values())[
                        -1 * len(client_GENs2[i].state_dict()) :
                    ],
                )
                zip1 = dict(zip1)
                ...
                zip1 = zip(
                    client_Disc1[i].state_dict().keys(),
                    list(all_layers_disc.values())[: len(client_Disc1[i].state_dict())],
                )
                zip1 = dict(zip1)
                client_Disc1[i].load_state_dict(zip1)
                zip1 = zip(
                    client_Disc2[i].state_dict().keys(),
                    list(all_layers_disc.values())[
                        -1 * len(client_Disc2[i].state_dict()) :
                    ],
                )
                zip1 = dict(zip1)
                client_Disc2[i].load_state_dict(zip1)

    server_GEN.load_state_dict(
        dict(
            list(all_layers_gen2.items())[
                len(client_GENs1[smallest_components_clients[0]].state_dict()) : -1
                * len(client_GENs2[smallest_components_clients[1]].state_dict())
                - 1
            ]
        )
    )
    server_Disc.load_state_dict(
        dict(
            list(all_layers_disc2.items())[
                len(client_Disc1[smallest_components_clients[2]].state_dict()) : -1
                * len(client_Disc2[smallest_components_clients[3]].state_dict())
                - 1
            ]
        )
    )
